...
def gravar_historico(sufixo, valor, limite_registros=250):
    ...
    logger.info(f"PASSOU AQUI LIMIT=0: {nome_classe}")
    time.sleep(1)
    response = connection.getresponse()
    logger.info(f"Response status: {response.status}")  # Adiciona log do status da resposta
    data = response.read()
    logger.info(f"Dados da resposta: {data}") 
    total_registros = json.loads(data).get('count', 0)
    logger.info(f"total_registros: {total_registros}")
